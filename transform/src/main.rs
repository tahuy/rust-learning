...
fn read_json_file(path: &str) -> SalesAndProducts {
...
fn read_xml_file(path: &str, sales_and_products: &mut SalesAndProducts) {
...
fn write_into_redis_store(
    ...
    for product in &sales_and_products.products {
        ...
        conn.set(
            format!("product:{}:name", product.id), &product.name)?;
    }
    for sale in &sales_and_products.sales {
        conn.set(
            format!("sale:{}:product_id", sale.id), sale.product_id)?;
        conn.set(
            format!("sale:{}:sale_date", sale.id), sale.date)?;
        conn.set(
            format!("sale:{}:quantity", sale.id), sale.quantity)?;
        conn.set(
            format!("sale:{}:unit", sale.id), &sale.unit)?;
    }
    Ok(())
}
...
fn get_from_id(conn: &mut postgres::Client, id: i32) -> Result<(), postgres::error::Error> {
    println!("List information of product:");
    for row in &conn.query("SELECT * FROM Products WHERE id = $1", &[&id])? {
...
fn main() {
    ...
    let config: Config = {
        let config_text =
            std::fs::read_to_string(&options.file).unwrap();
    ...
    let sqlite_conn =
        create_sqlite_db(&config.sqlite).unwrap();
    write_into_sqlite_db(&sqlite_conn, &sales_and_products).unwrap();

    let mut postgresql_conn =
        create_postgresql_db(&config.postgresql).unwrap();
    ...
    match &options.id {
        Some(x) => get_from_id(&mut postgresql_conn, *x).unwrap(),
        None => println!("No id"),
    };
}